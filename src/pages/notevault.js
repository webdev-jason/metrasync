// --- MODULE STATE (Persists across dashboard navigations) ---
...
export function init() {
    // 1. Clean up old window listeners from previous tool visits
    if (abortController) abortController.abort();
    abortController = new AbortController();
    const signal = abortController.signal;

    // 2. Map fresh DOM elements
    els = {
    ...
    function setTheme(theme) {
      const wrapper = document.querySelector('.notevault-wrapper');
      if (theme === 'light') {
    ...
    function closeAppAlert() { els.appAlertModal.setAttribute("aria-hidden", "true"); }
    ...
    // --- CROPPER LOGIC ---
    let cropperState = { imageId: null, naturalWidth: 0, naturalHeight: 0, imgRect: null, crop: { x: 0, y: 0, size: 100 }, zoom: 1 };
    ...
    function closeCropper() { els.cropperModal.setAttribute("aria-hidden", "true"); }
    ...
    function positionCropBox(viewX, viewY, viewSize) {
      ...
      const x = Math.max(imageRect.left, Math.min(viewX, imageRect.right - size));
      const y = Math.max(imageRect.top, Math.min(viewY, imageRect.bottom - size));
    ...
    const interaction = { active: false, mode: null, startX: 0, startY: 0, startRect: null, resizeAnchor: { x: 0, y: 0 }, resizeCorner: null };
    ...
    window.addEventListener("pointermove", (e) => {
      ...
      if (interaction.mode === 'move') {
        ...
        const newSize = Math.max(20, Math.min(Math.min(imgRect.width, imgRect.height), Math.max(Math.abs(cx - interaction.resizeAnchor.x), Math.abs(cy - interaction.resizeAnchor.y))));
        let x = interaction.resizeAnchor.x;
        let y = interaction.resizeAnchor.y;
        if (interaction.resizeCorner === 'nw') { x -= newSize; y -= newSize; }
        if (interaction.resizeCorner === 'ne') { y -= newSize; }
        if (interaction.resizeCorner === 'sw') { x -= newSize; }
        positionCropBox(x, y, newSize);
      }
    }, { signal });

    window.addEventListener("pointerup", () => { interaction.active = false; interaction.mode = null; }, { signal });

    els.cropBox.addEventListener("wheel", (e) => {
      e.preventDefault();
      const delta = Math.sign(e.deltaY);
      const rect = els.cropBox.getBoundingClientRect();
      positionCropBox(rect.left, rect.top, rect.width * (1 - 0.08 * delta));
    }, { passive: false });

    els.cropperImage.addEventListener("click", (e) => {
      const cbRect = els.cropBox.getBoundingClientRect();
      positionCropBox(e.clientX - cbRect.width / 2, e.clientY - cbRect.width / 2, cbRect.width);
    });

    els.cropBox.addEventListener("keydown", (e) => {
      const rect = els.cropBox.getBoundingClientRect();
      const step = e.shiftKey ? 10 : 2;
      if (e.key === "ArrowLeft") { e.preventDefault(); positionCropBox(rect.left - step, rect.top, rect.width); }
      if (e.key === "ArrowRight") { e.preventDefault(); positionCropBox(rect.left + step, rect.top, rect.width); }
      if (e.key === "ArrowUp") { e.preventDefault(); positionCropBox(rect.left, rect.top - step, rect.width); }
      if (e.key === "ArrowDown") { e.preventDefault(); positionCropBox(rect.left, rect.top + step, rect.width); }
      if (e.key === "-" || e.key === "_") { e.preventDefault(); positionCropBox(rect.left, rect.top, rect.width * 0.92); }
      if (e.key === "=" || e.key === "+") { e.preventDefault(); positionCropBox(rect.left, rect.top, rect.width * 1.08); }
    });
    ...
    function syncCropControls() {
      ...
      els.cropSizeRange.max = String(Math.floor(Math.min(stage.width, stage.height)));
      els.cropSizeRange.value = String(Math.floor(els.cropBox.getBoundingClientRect().width));
    }

    els.cropMinusBtn?.addEventListener("click", () => { const r = els.cropBox.getBoundingClientRect(); positionCropBox(r.left, r.top, r.width * 0.92); });
    els.cropPlusBtn?.addEventListener("click", () => { const r = els.cropBox.getBoundingClientRect(); positionCropBox(r.left, r.top, r.width * 1.08); });
    els.cropCenterBtn?.addEventListener("click", () => {
      const stage = getDisplayedImageRect();
      const size = els.cropBox.getBoundingClientRect().width;
      positionCropBox(stage.left + (stage.width - size) / 2, stage.top + (stage.height - size) / 2, size);
    });
    els.cropSizeRange?.addEventListener("input", (e) => { const r = els.cropBox.getBoundingClientRect(); positionCropBox(r.left, r.top, Number(e.target.value) || r.width); });

    function setZoom(z) {
      cropperState.zoom = Math.max(0.25, Math.min(z, 3));
      els.cropperImage.style.setProperty('--crop-zoom', String(cropperState.zoom));
      const rect = els.cropBox.getBoundingClientRect();
    ...
    function getDisplayedImageRect() {
      ...
      const w = natW * fit, h = natH * fit;
      const left = stage.left + (stage.width - w) / 2, top = stage.top + (stage.height - h) / 2;
    ...
    els.imageViewerModal.addEventListener("click", (e) => { if (e.target === els.imageViewerModal) closeImageViewer(); });
    els.imageViewerModal.addEventListener("wheel", (e) => {
      e.preventDefault(); 
      imageViewerZoom = Math.max(0.5, Math.min(imageViewerZoom + (e.deltaY > 0 ? -0.1 : 0.1), 5));
      els.modalViewerImage.style.transform = `scale(${imageViewerZoom})`;
    }, { passive: false });

    // --- CONTEXT BRIDGE IPC CALLS (Replaces require('electron')) ---
    ...
    window.api.onDataLoaded(async (jsonContent) => {
      try {
    ...
    async function saveNotes() {
      try { await db.set(CURRENT_PROFILE_KEY, notes); } 
      catch (e) { showAppAlert("Database Error", "FATAL ERROR: Could not save notes. " + e.message); }
    }

    function updateProfileUI() { els.currentProfileBtn.textContent = `Profile: ${CURRENT_PROFILE_KEY}`; }

    async function switchProfile(newKey, close = true) {
      if (newKey === CURRENT_PROFILE_KEY) { if (!close) renderProfileList(); return; }
      await saveNotes(); 
      ...
      else { renderProfileList(); els.newProfileInput.value = ""; els.newProfileInput.focus(); }
    }

    function deleteProfile(keyToDelete) {
      if (keyToDelete === CURRENT_PROFILE_KEY) { showAppAlert("Cannot Delete", "You cannot delete the active profile."); return; }
      showAppConfirm("Delete Profile?", `Are you sure you want to delete profile "${keyToDelete}"?`, async () => {
        await db.deleteKey(keyToDelete);
        await renderProfileList(); 
        setTimeout(() => { if (els.newProfileInput) els.newProfileInput.focus(); }, 50);
      }, true);
    }
    ...
    async function createProfile() {
      ...
      if (keys.includes(safeName)) { showAppAlert("Error", "Profile name already exists."); return; }
    ...
    async function renderProfileList() {
      ...
      const keys = await db.getAllKeys();
      keys.forEach(key => {
        if (key === 'default_notes' || key === 'all_notes') return; 
        ...
        item.appendChild(switchBtn); item.appendChild(delBtn);
        els.profileList.appendChild(item);
      });
    }

    function openProfileModal() { renderProfileList(); els.profileModal.setAttribute("aria-hidden", "false"); }
    function closeProfileModal() { els.profileModal.setAttribute("aria-hidden", "true"); }
    ...
    function generateId(prefix) { return `${prefix}_${Math.random().toString(36).slice(2, 9)}_${Date.now().toString(36)}`; }

    function ensureSelection() {
      const note = notes.find((n) => n.id === selectedNoteId);
      if (!note) {
        const filterText = els.searchInput.value.toLowerCase().trim();
        const filteredNotes = notes.filter(n => filterText === "" || n.partId.toLowerCase().startsWith(filterText));
    ...
    function createNoteContentView(note) {
      ...
      partId.className = "print-part-id"; partId.textContent = note.partId || "Untitled";
      frag.appendChild(partId);
      const body = document.createElement("div");
      body.className = "note-body"; body.textContent = note.body || "";
      ...
      note.images.forEach((img) => {
        ...
        frame.appendChild(image); card.appendChild(frame);
        const captionText = (img.caption || "").trim();
        if (captionText.length > 0) {
          const cap = document.createElement("div");
          cap.className = "caption"; cap.textContent = captionText;
          card.appendChild(cap);
        }
        imagesWrap.appendChild(card);
        card.addEventListener("click", () => { els.modalViewerImage.src = img.dataUrl; els.imageViewerModal.setAttribute("aria-hidden", "false"); });
      });
    ...
    function renderNotesList() {
      ...
      notes
        .filter(note => filterText === "" || note.partId.toLowerCase().startsWith(filterText))
        .sort((a, b) => (a.partId || "").toLowerCase().localeCompare((b.partId || "").toLowerCase(), undefined, { numeric: true, sensitivity: 'base' }))
        .forEach((note) => {
          ...
          scaleWrapper.appendChild(createNoteContentView(note));
          thumb.innerHTML = ""; thumb.appendChild(scaleWrapper);
          ...
          if (title) {
            ...
            requestAnimationFrame(() => {
              ...
              while (textWidth > title.clientWidth && fontSize > 8) {
                fontSize--; title.style.fontSize = `${fontSize}px`; textWidth = title.scrollWidth;
              }
            });
          }
          clone.addEventListener("click", () => { selectedNoteId = note.id; selectedImageId = null; renderAll(); });
        });
    }
    ...
    function renderViewer() {
      ...
      els.viewer.appendChild(createNoteContentView(note));
    }
    ...
    function renderEditor() {
      ...
      if (isNew) { els.partIdInput.value = ""; els.notesInput.value = ""; els.imagesContainer.innerHTML = ""; return; }
      els.partIdInput.value = note.partId; els.notesInput.value = note.body; els.imagesContainer.innerHTML = "";
      note.images.forEach((img) => {
        const card = document.createElement("div"); card.className = "img-card";
        if (img.id === selectedImageId) card.classList.add("active");
        const image = document.createElement("img"); image.src = img.dataUrl;
        const caption = document.createElement("div"); caption.className = "caption"; caption.textContent = img.caption || "";
        const actions = document.createElement("div"); actions.className = "img-actions";
        const cropBtn = document.createElement("button"); cropBtn.textContent = "Crop"; cropBtn.type = "button";
        const delBtn = document.createElement("button"); delBtn.textContent = "Delete"; delBtn.className = "danger"; delBtn.type = "button";
        actions.appendChild(cropBtn); actions.appendChild(delBtn);
        card.appendChild(image); card.appendChild(actions); card.appendChild(caption);
        card.addEventListener("click", () => { selectedImageId = img.id; els.imageCaptionInput.value = img.caption || ""; renderEditor(); });
        cropBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); openCropper(img); });
        delBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); removeImage(img.id); });
    ...
    function renderAll() { ensureSelection(); renderNotesList(); renderViewer(); renderEditor(); updateProfileUI(); }

    async function createNote() {
      const newNote = { id: generateId("note"), partId: "", createdAt: Date.now(), updatedAt: Date.now(), body: "", images: [] };
      notes.unshift(newNote); selectedNoteId = newNote.id; selectedImageId = null;
      await saveNotes(); renderAll();
    }

    async function saveCurrentNote() {
      const note = notes.find((n) => n.id === selectedNoteId);
      if (!note) return;
      note.partId = els.partIdInput.value.trim() || "Untitled"; note.body = els.notesInput.value; note.updatedAt = Date.now();
      await saveNotes(); renderAll();
    }

    function deleteCurrentNote() {
      if (!selectedNoteId) return;
      showAppConfirm("Delete Note?", "Are you sure you want to delete this note? This action cannot be undone.", async () => {
        const idx = notes.findIndex((n) => n.id === selectedNoteId);
        if (idx >= 0) notes.splice(idx, 1);
        selectedNoteId = null; selectedImageId = null;
        await saveNotes(); renderAll();
      }, true);
    }
    ...
    async function copyCurrentNote() {
      ...
      const newImages = originalNote.images.map(img => {
        ...
        if (img.lastCrop) clone.lastCrop = { ...img.lastCrop }; return clone;
      });
      const newNote = { ...originalNote, id: generateId("note"), partId: `${originalNote.partId} (COPY)`, images: newImages, createdAt: Date.now(), updatedAt: Date.now() };
      notes.push(newNote); selectedNoteId = newNote.id; selectedImageId = null; 
      await saveNotes(); renderAll();
      setTimeout(() => { const list = document.getElementById("notesList"); if (list) list.scrollTop = list.scrollHeight; }, 50);
    }
    ...
    async function addImagesFromFiles(fileList) {
      ...
      if (oversized.length > 0 && !confirm(`${oversized.length} file(s) are larger than 50MB. Continue anyway?`)) { els.imageInput.value = ""; return; }
      
      let pending = files.length, hasError = false;
      files.forEach((file) => {
        ...
        reader.onload = async () => {
          try {
            ...
            note.images.push({ id: generateId("img"), dataUrl: compressed, originalDataUrl: compressed, caption: "" });
            if (--pending === 0 && !hasError) {
              note.updatedAt = Date.now();
              try { await saveNotes(); renderAll(); } 
              catch (err) { alert("Error saving notes. Image likely too large."); }
            }
          } catch (err) {
            hasError = true; if (--pending === 0) { alert("Error adding one or more images."); renderAll(); }
          }
        };
    ...
    async function compressDataUrl(dataUrl, opts) {
      ...
      try {
        ...
        const ctx = canvas.getContext('2d');
        ctx.imageSmoothingQuality = 'high'; ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
    ...
    async function removeImage(imageId) {
      ...
      note.updatedAt = Date.now(); await saveNotes(); renderAll();
    }
    ...
    const triggerAutosave = debounce(async () => { await saveNotes(); }, 1000);
    function handleNoteInput() {
      const note = notes.find((n) => n.id === selectedNoteId);
      if (!note) return;
      note.partId = els.partIdInput.value; note.body = els.notesInput.value; note.updatedAt = Date.now();
      triggerAutosave();
    }
    ...
    els.imageCaptionInput.addEventListener("input", (e) => {
      ...
      if (!img) return;
      img.caption = e.target.value; note.updatedAt = Date.now(); triggerAutosave();
    ...
    els.imagesContainer.addEventListener("wheel", (e) => { if (e.deltaY !== 0) { e.preventDefault(); els.imagesContainer.scrollLeft += e.deltaY; } }, { passive: false });
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          if (Array.isArray(parsed)) {
             await db.set('default_notes', parsed);
      ...
      if (lastUsed && keys.includes(lastUsed)) { CURRENT_PROFILE_KEY = lastUsed; } 
      else if (keys.length > 0) { CURRENT_PROFILE_KEY = keys[0]; localStorage.setItem('last_profile', CURRENT_PROFILE_KEY); } 
      else { CURRENT_PROFILE_KEY = 'default_notes'; await db.set(CURRENT_PROFILE_KEY, []); localStorage.setItem('last_profile', CURRENT_PROFILE_KEY); }
